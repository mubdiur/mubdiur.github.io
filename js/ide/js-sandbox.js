/* ═══════════════════════════════════════════════════════════
   ...
   Protocol: { code } in → { type:'out'|'err'|'exit', text, code } out.
   ═══════════════════════════════════════════════════════════ */
'use strict';
...
var builtins = {
  ...
  crypto: {
    randomUUID: function () {
      ...
      return Array.from(b, function (x, i) { return i === 4 || i === 6 || i === 8 || i === 10 ? '-' + x.toString(16).padStart(2, '0') : x.toString(16).padStart(2, '0'); }).join('');
    },
...
self.addEventListener('message', function (e) {
  ...
  try {
    var fn = new Function(
      ...
      'clearTimeout', 'clearInterval', 'queueMicrotask', 'fetch',
    ...
    var p = fn(
      ...
      sandbox.queueMicrotask, sandbox.fetch,
  ...
  setTimeout(function () {
    if (running) {
      post('err', '\n[terminated: execution exceeded ' + Math.round(PENDING_TIMEOUT / 1000) + 's — check for infinite loops]\n');
