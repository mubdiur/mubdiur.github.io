...
App.registerTool('contrast-checker', {
  css: '' +
    '.t-contrast-checker .color-input{display:block;width:100%;height:2.5rem;border:1px solid rgba(80,73,69,0.4);border-radius:var(--radius);margin-top:0.25rem;cursor:pointer;}\n' +
    '.t-contrast-checker .hex-input{width:100%;height:1.75rem;margin-top:0.25rem;border:1px solid rgba(80,73,69,0.4);background:rgba(0,0,0,0.3);padding:0 0.5rem;font-size:10px;font-family:var(--font-mono);color:rgba(235,219,178,0.8);border-radius:var(--radius);outline:none;transition:border-color .2s,box-shadow .2s;}\n' +
    '.t-contrast-checker .hex-input:focus{border-color:rgba(142,192,124,0.4);box-shadow:0 0 0 1px rgba(142,192,124,0.3);}\n' +
    '.t-contrast-checker .text-muted-foreground\\/60{color:rgba(189,174,147,0.6);}\n' +
    '.t-contrast-checker .text-muted-foreground\\/70{color:rgba(189,174,147,0.7);}\n' +
    '.t-contrast-checker .bg-black\\/10{background:rgba(0,0,0,0.1);}\n' +
    '.t-contrast-checker .text-green-glow\\/80{color:rgba(184,187,38,0.8);}\n' +
  ...
  mount: function (root) {
    ...
    function render() {
      ...
      resultsBox.appendChild(App.el('div', { class: 'flex justify-between items-center py-1.5 px-2 rounded bg-black/20' },
        ...
        App.el('span', { class: 'text-lg font-bold', style: { color: parseFloat(result.ratio) >= 4.5 ? '#34d399' : '#f87171' }, text: result.ratio + ':1' })));
