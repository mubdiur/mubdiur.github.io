...
function TransformToolUI(tool) {
  ...
  (tool.params || []).forEach(function (p) {
    ...
    if (p.type === 'select') {
      ...
      control = App.el('input', { type: 'checkbox', class: 't-checkbox', 'aria-label': p.label, style: 'width:16px;height:16px;accent-color:#94e2d5' });
    ...
    paramsRow.appendChild(App.el('div', { class: 'flex items-center gap-1.5' },
      App.el('label', { class: 'text-xs font-mono', style: { color: 'rgba(166,173,200,0.7)', whiteSpace: 'nowrap' }, text: p.label }),
      control));
  });
  ...
  var root = App.el('div', { class: 'flex flex-col', style: { gap: '1rem' } },
    paramsRow,
    App.el('div', {},
      App.el('div', { class: 'text-xs font-mono mb-1', style: { color: 'rgba(166,173,200,0.7)' }, text: 'Input' }),
      inputTa),
    outputArea);
