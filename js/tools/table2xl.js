...
function buildHeader(table, headerTexts, keepIndices) {
  ...
  keepIndices.forEach(function (idx) {
    ...
    th.style.cssText = 'background:#1e1e22;font-weight:700;border:1px solid #1e1e22;padding:4px 8px;text-align:left;color:#e8e8ea';
    tr.appendChild(th);
  });
...
function buildTable(table, rows, cellSelector, keepIndices) {
  ...
  rows.forEach(function (row) {
    ...
    indices.forEach(function (idx, visibleIdx) {
      ...
      td.textContent = text;
      td.style.cssText = 'border:1px solid #1e1e22;padding:4px 8px;text-align:left;color:#e8e8ea';
      tr.appendChild(td);
    });
...
App.registerTool('table2xl', {
  css: '' +
    ...
    '.t-table2xl .fg-90{color:rgba(232,232,234,0.9);}\n' +
    '.t-table2xl .muted-80{color:rgba(168,168,176,0.8);}\n' +
    '.t-table2xl .muted-70{color:rgba(168,168,176,0.7);}\n' +
    '.t-table2xl .muted-60{color:rgba(168,168,176,0.6);}\n' +
    '.t-table2xl .hover-fg-90:hover{color:rgba(232,232,234,0.9);}\n' +
    '.t-table2xl .seg{display:flex;gap:0.25rem;border:1px solid rgba(30,30,34,0.5);border-radius:0.5rem;background:var(--mantle);padding:0.25rem;}\n' +
    '.t-table2xl .seg button{flex:1;padding:0.375rem 0.75rem;border-radius:0.375rem;font-family:var(--font-mono);font-size:12px;border:1px solid transparent;background:none;color:rgba(168,168,176,0.7);cursor:pointer;transition:color .2s,background-color .2s,border-color .2s;}\n' +
    '.t-table2xl .seg button:hover{color:rgba(232,232,234,0.9);}\n' +
    '.t-table2xl .seg button.active{background:rgba(194,194,200,0.2);color:var(--ctp-teal);border-color:rgba(194,194,200,0.3);}\n' +
